...
| 한국 빅테크 풀 변경 (KR_TECH_POOL / KR_MOVERS_POOL) | snapshot.js BIGTECH의 KR_TECH region 종목과 1:1 일치 필요. render.js: KR_TECH_POOL(시총 10) → pickTopMovers \|Δ\| top 3 + KR_MOVERS_POOL(변동성 5) → pickMaxGain 1 + pickMaxLoss 1, dedupe 후 5개 카드. pool 카드 모두 fetch(빅테크 yahoo 1y) 되므로 너무 많아지면 subrequest 한도 영향 — 현재 KR 15 + US 5 = 20개, 거시 18 합산 38, 안전 margin 적음 |
